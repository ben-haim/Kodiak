	...
	public static void main(String[] args) throws Exception
	{
		...
		while((line = reader.readLine()) != null)
		{
			...
			if(PRINT_START_TIME.getTime() <= time.getTime() && time.getTime() <= PRINT_END_TIME.getTime())
			{
				...
				for(Map.Entry<String, Quote> entry : bidQuotes.entrySet())
				{
					System.out.println("BID," + entry.getKey() + "," + entry.getValue().getBidPrice()+","+entry.getValue().getBidSize()+","+new Date(entry.getValue().getTimestamp()));
				}
				for(Map.Entry<String, Quote> entry : askQuotes.entrySet())
				{
					System.out.println("ASK," + entry.getKey() + "," + entry.getValue().getAskPrice()+","+entry.getValue().getAskSize()+","+new Date(entry.getValue().getTimestamp()));
				}
				System.out.println();
			}
