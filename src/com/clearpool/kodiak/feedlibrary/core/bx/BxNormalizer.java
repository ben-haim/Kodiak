...
public class BxNormalizer implements IMdNormalizer
{
	...
	public BxNormalizer(Map<MdServiceType, IMdLibraryCallback> callbacks, String range)
	{
		this.bookCache = new BookQuoteCache((IMdBookQuoteListner) callbacks.get(MdServiceType.BOOK_XBOS), MdFeed.BX, MdServiceType.BOOK_XBOS, range);
		this.imbalanceCache = new ImbalanceCache((IMdImbalanceListener) callbacks.get(MdServiceType.IMBALANCE_XBOS), MdFeed.BX, MdServiceType.IMBALANCE_XBOS, range);
	...
	{
		BxPacket bxPacket = (BxPacket) packet;
		ByteBuffer buffer = bxPacket.getBuffer();
		...
		if (messageType == ADD_ORDER_NO_MPID || messageType == ADD_ORDER_WITH_MPID)
		{
			...
			this.bookCache.addOrder(symbol, orderReferenceNumber, side, (int) shares, price, Exchange.USEQ_NASDAQ_OMX_BX.getMicCode(), timestamp);
		}
		...
		else if (messageType == SYSTEM_EVENT)
		{
			...
			switch (eventCode)
			{
				...
				case 'C':
					LOGGER.info("End of System Hours");
					bxPacket.setEndOfTransmission(true);
					break;
				default:
					break;
			}
		...
		else if (messageType == NOII)
		{
			...
			this.imbalanceCache.updateImbalance(symbol, pairedShares, imbalanceShares, imbalanceSide, 0, currentReferencePrice, nearPrice, farPrice, 0, Exchange.USEQ_NASDAQ_OMX_BX, auctionType, timestamp);
		}
